...
export default function Home() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await fetch(`http://localhost:8088/api/Workout`);
  ...
  const handleDeleteUser = async () => {
    try {
      const res = await fetch(
        `http://localhost:8088/api/WorkoutDelete/${ItemDelete}`,
        {
          method: "DELETE",
        }
      );
...
<div className="absolute transform -translate-x-0 translate-y-0 top-1  ml-6 ">
    ...
    <Link to="/createschedul">
        <button className="bg-gradient-to-r from-slate-500 to-blue-white bg-opacity-80  w-32 h-10 border border-slate-300 shadow-xl rounded-lg text-white hover:opacity-85 ml-[1240px]  font-extralight  text-opacity-75">New Workout</button>
        </Link>
        </div>
        ...
        {workouts.map((workout) => (
          ...
          >
            <div className="px-6 py-4">
                ...
                WorkOut Status: {workout.workoutState}
              </h2>
              ...
              <div className="max-h-44 overflow-y-auto  scrollbar-none bg-slate-600 rounded-xl bg-opacity-10">
                {workout.state.map((state, index) => (
                  <div key={index} className="gap-2 ml-4   rounded-xl  ">
                   ...
                   <div className="ml-4">{state.name}</div>
                    </div>
