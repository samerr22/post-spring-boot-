...
export default function CreateSchedul() {
  ...
  useEffect(() => {
    try {
      const fetchworkout = async () => {
        const res = await fetch(`http://localhost:8088/api/Workout?itemId=${workId}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:8088/api/WorkoutUp/${formData.id}`, {
        method: "PUT",
        headers: {
...
<div className="absolute transform -translate-x-0 translate-y-0 top-1  ml-[800px] ">
      ...
      <Link to="/Myschedul">
        <button className="bg-gradient-to-r from-slate-500 to-blue-white   bg-opacity-80  w-32 h-10 border border-slate-300 shadow-xl rounded-lg text-white hover:opacity-85 ml-[800px]  font-extralight  text-opacity-75">My Workout</button>
        </Link>
        </div>
      ...
      <div className="flex p-3 max-w-3xl mx-auto flex-col md:flex-row md:items-center gap-5">
        ...
        <div className="flex justify-center items-center mt-6">
          <form className="flex flex-col  gap-4" onSubmit={handleSubmit} >
            ...
            <div>
            <h3 className="font-semibold  ml-1 text-black">Workout</h3>
            <select
          ...
          >
            ...
            <div>
             <h3 className="font-semibold ml-1 text-black ">Date</h3>
          ...
          <button className=" bg-gradient-to-r from-slate-500 to-blue-white   bg-opacity-80 text-white border border-slate-300 shadow-xl shadow-slate-400 font-serif  text-opacity-90 p-3 rounded-lg w-[460px] h-11 hover:opacity-90" type="button" onClick={handleAddExercise}>Update Current Exercise</button>
            <button
