 ...
 console.log(workouts)
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await fetch(`http://localhost:8088/api/Get`);
  ...
  const handleDeleteUser = async () => {
    ...
    while (attempts < maxRetries) {
      try {
        const res = await fetch(`http://localhost:8088/api/delete/${ItemDelete}`, {
          method: "DELETE",
        });
  ...
  const handleLike = async (postId) => {
    try {
        const res = await fetch(`http://localhost:8088/api/like/${postId}`, {
            method: "PUT",
            headers: {
...
const handleComment = async (postId, commentText) => {
  try {
    const res = await fetch(`http://localhost:8088/api/comment/${postId}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
<div className="absolute transform -translate-x-0 translate-y-0 top-0   bg-[#d9d9da] bg-opacity-95  ml-[480px] ">

<Link to="/create">
        <button className="bg-white  bg-opacity-80  w-32 h-10 border mt-2 border-slate-300 shadow-xl rounded-lg text-black hover:opacity-85  font-extralight  text-opacity-75">New post</button>
        </Link>
    ...
    <div className="flex justify-center mt-0">
      ...
      <div className="max-h-[655px] w-full overflow-y-auto  scrollbar-none">
        {workouts.map((workout) => (
          <div
            key={workout.id}
            className="w-[550px] h-[720px] bg-white mt-10 mb-5 border-none rounded-2xl  relative z-10 "
          >
